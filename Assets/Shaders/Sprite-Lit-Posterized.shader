    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 CombinedShapeLightFragment(Varyings i) : SV_Target
            {
                ...
                // ดึงค่า light
                half light = max(litColor.r, max(litColor.g, litColor.b));

                // ❗ clamp ไม่ให้สว่างเกิน
                light = light / (1.0 + light);

                // posterize (ถ้ามี)
                float steps = max(1.0, _PosterizeSteps);
                light = floor(light * steps) / (steps - 1.0);

                // apply
                litColor.rgb = surfaceData.albedo.rgb * light;
                return litColor;
